...
class Ground(private val noise: Noise = Simplex()) : Actor() {
    private var bt = Texture(Gdx.files.internal("ground.png"))
    private var shader = ShaderProgram(Gdx.files.internal("ground.vs"),
            Gdx.files.internal("ground.fs"))

    private var mesh: Mesh
    private var cam = Matrix4()
    var scroll = Vector2(0.0f, 0.0f)
    private var steepness = 500.0f
    private var minh = 0.4f
    private var maxh = 0.6f

    private var tr = TextureRegion()
    ...
    operator fun plusAssign(d: Float) {
        ...
        if (steepness > 300f)
            steepness -= d / 200.0f
        minh -= d / 110.0f / GameScreen.worldHeight
        if (minh < 0.1f)
            minh = 0.1f
        maxh += d / 70.0f / GameScreen.worldHeight
